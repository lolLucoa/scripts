...
Domain.JoinCodes.CreateJC.MouseButton1Click:Connect(function()
	if UserIsPro then
		...
		local result = tostring(game.PlaceId ..":".. game.JobId)
		...
		if result ~= "fail" then
			...
			Domain.CreateCode.GeneratedCode.TextScaled = true
...
Domain.JoinCodes.CodeBox.JCText.TextScaled = true
...
Domain.JoinCodes.CodeBox.JCText.FocusLost:Connect(function()
	...
	if result ~= "invalid" and result ~= "fail" then
		local decoded = string.split(result, ":")
		local placeId = decoded[1]
		local serverId = decoded[2]
